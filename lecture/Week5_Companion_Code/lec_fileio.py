...
# Close the file
fobj.close()   #Important to close the file
print(fobj.closed)
...
with open(SRCFILE, mode='r') as fobj:  #context manager
   cnts = fobj.read()
...
with open(DSTFILE, mode='a') as fobj:       # mode = 'w' will erase the previous one
...
FINDFILE = os.path.join(cfg.DATADIR, 'iso.txt')
with open(FINDFILE, mode='r') as fobj1:
    NEWLIST = []
    for line in fobj1:
        line = line.split()
        NEWLIST = NEWLIST + line
    d = {x: NEWLIST.count(x) for x in NEWLIST}
    HIGHEST_FRE_WORD = max(d, key = lambda w: d[w])
    HIGHEST_FRE = max(d.values())
    print(f'The highest frequency word is {HIGHEST_FRE_WORD} and its frequency is {HIGHEST_FRE}.')
