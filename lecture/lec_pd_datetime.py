...
new_delta  = dt.datetime(year = 2023, month = 1, day = 1, hour = 0) - dt.datetime(year = 2022, month = 12, day = 31, hour = 12)
print(new_delta)
...
s  = date.strftime('%A of week number %U of the year %Y')
print(s)
...
#
prc = pd.read_csv(CSVLOC, parse_dates=['Date'], index_col='Date')
prc_new = prc.loc['2019-03', "Adj Close"].iloc[14]
print(prc_new1)
...
# compute returns
prc["Percentage change"] = prc.loc[:, 'Close'].pct_change()
print(prc)

prc1 = pd.read_csv(CSVLOC)
prc1.info()

prc2 = pd.read_csv(CSVLOC, parse_dates=['Date'])
print(prc2.head())
prc3 = pd.read_csv(CSVLOC, parse_dates=['Date'], index_col='Date')
print(prc3.head())